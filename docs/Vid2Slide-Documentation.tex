...
\definecolor{dark}{RGB}{9, 9, 11}
\definecolor{surface}{RGB}{24, 24, 27}

% Custom commands
\newcommand{\sectionTitle}[1]{\section*{\textbf{#1}}}
\newcommand{\subsectionTitle}[1]{\subsection*{\textbf{#1}}}
...
\begin{enumerate}
    \item Go to \textbf{Settings} $\rightarrow$ \textbf{About}
...
\begin{figure}[H]
    \centering
    \textbf{[Main Interface Layout]}\par\medskip
    \textit{(1) Toolbar | (2) Video Preview | (3) Processing Controls | (4) Slide Gallery}
\end{figure}
...
\textbf{How it works:}
\begin{enumerate}
    \item Reduce image to a small size (32×32 pixels)
...
Two frames with a Hamming distance $\leq$ 5\% are considered duplicates.
...
\textbf{Formula:}
\[
\text{Variance} = \frac{\sum_{i=1}^{N}(I[i] - \mu)^2}{N}
\]

Where $I$ is the Laplacian-transformed pixel values and $\mu$ is the mean value.
...
\begin{enumerate}
    \item Open \textbf{Settings} $\rightarrow$ \textbf{Processing}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \midrule
        pHash calculation & ✓ & ✓ (10x faster) \\
        Blur detection & ✓ & ✓ (8x faster) \\
        Image loading & ✓ & ✓ (3x faster) \\
        PDF generation & ✓ & - \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{enumerate}
    \item Go to \textbf{File} $\rightarrow$ \textbf{Batch Mode}
...
Tips for deletion:
\begin{itemize}
    \item Delete duplicate slides even if threshold didn't catch them
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        Draft & 720p (1280×720) & ~500 KB & Quick preview, sharing \\
        Standard & 1080p (1920×1080) & ~2 MB & General use \\
        High & 2K (2560×1440) & ~5 MB & Print quality \\
...
\begin{itemize}
    ...
    \item \textbf{Language}: UI display language
\end{itemize}
...
\begin{enumerate}
    \item Verify the file isn't corrupted (try opening in VLC)
    \item Update video codecs (install K-Lite Codec Pack)
    \item Check file read permissions
    \item Ensure sufficient disk space ($\sim$2x video size)
\end{enumerate}
...
\begin{enumerate}
    \item Go to Settings $\rightarrow$ Advanced
...
\textbf{Q: My video is very long (>3 hours). Will it take longer?} \\
...
A: A technique that generates similar hashes for visually similar images, allowing detection of duplicate slides even when they're not pixel-perfect matches.
...
\textbf{Q: What's the maximum video length?} \\
A: There's no hard limit, but very long videos (8+ hours) may require significant processing time and disk space. Consider splitting long videos.
...
\begin{enumerate}
    \item Vid2Slide version (Help $\rightarrow$ About)
...
\begin{itemize}
    ...
    \item \textbf{Feedback Form}: Available in Help $\rightarrow$ Send Feedback
\end{itemize}
...
\begin{center}
\textbf{\large © 2024 EPR Groupers. All rights reserved.}\\[0.5em]
